...
/**
 * MySQL2 Storage Adapter.
 *
 * @remarks
 *
 * This adapter uses 'mysql2' to provide a MySQL database. You must install this package manually.
 *
 * @category Adapter
 */
export class MySQLAdapter extends SQLAdapter {
  protected override readonly options: MySQL2Options;

  /**
   * Initialize MySQL2 storage adapter.
   *
   * @remarks
   *
   * This adapter uses 'mysql2' to provide a MySQL database. You must install this package manually.
   *
   * useNullAsDefault is not available for modification. It will always be set to true due to MySQL driver limitations.
   *
   * @param options The options used to configure the adapter.
   */
  public constructor(options: MySQL2Options & { useNullAsDefault?: boolean; }) {
    super();
    options.client = (options.client === 'mysql' || options.client === 'mysql2' ? options.client : 'mysql2');
  ...
  /**
   * Configure the MySQL storage adapter connection.
   *
   * @remarks
   *
