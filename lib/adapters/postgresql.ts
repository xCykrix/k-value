...
import type { KValueTable, MySQL2Options, PostgreSQLOptions } from '../abstraction/adapter'
...
export class PostgreSQLAdapter extends Adapter {
  private readonly options: PostgreSQLOptions & CacheOptions
  private readonly driver: KnexHandler

  public constructor (options: PostgreSQLOptions & CacheOptions & { useNullAsDefault: boolean; }) {
    super()
  ...
  public async configure (): Promise<void> {
    // Initialize Table
    if (!(await this.driver.knex.schema.hasTable(this.options.table ?? 'kv_global'))) {
      await this.driver.knex.schema.createTable(this.options.table ?? 'kv_global', (table) => {
  ...
  public async clear (): Promise<void> {
    await this.driver.knex(this.options.table ?? 'kv_global').delete()
    this.memcache.clear()
  }
  ...
  public async delete (id: string | string[]): Promise<void> {
    ...
    if (Array.isArray(id)) {
      // Delete ID List
      for (const i of id) {
        await this.driver.knex.table(this.options.table ?? 'kv_global').where({ key: i }).delete()
        this.memcache.delete(i)
      }
      // Delete Single ID
    } else {
      await this.driver.knex.table(this.options.table ?? 'kv_global').where({ key: id }).delete()
  ...
  public async get (id: string | string[], options?: GetOptions & CacheOptions): Promise<unknown | unknown[]> {
    ...
    if (Array.isArray(id)) {
      ...
      for (const k of id) {
        ...
        const state = await this.driver.knex(this.options.table ?? 'kv_global').select('*').where({ key: k }).first() as KValueTable
      ...
      const state = await this.driver.knex(this.options.table ?? 'kv_global').select('*').where({ key: id }).first() as KValueTable
      const context = super._import(state)
  ...
  public async has (id: string | string[]): Promise<boolean | Array<{ key: string; has: boolean; }>> {
    ...
    if (Array.isArray(id)) {
      ...
      for (const k of id) {
        result.push({
          key: k,
          has: await this.driver.knex(this.options.table ?? 'kv_global').select('key').where({ key: k }).first() !== undefined
        })
      }
      return result
    } else return await this.driver.knex(this.options.table ?? 'kv_global').select('key').where({ key: id }).first() !== undefined
  }

  public async keys (limits?: LimiterOptions): Promise<string[]> {
    let keys = (await this.driver.knex(this.options.table ?? 'kv_global').select('key') as KValueTable[]).map((v) => v.key)
