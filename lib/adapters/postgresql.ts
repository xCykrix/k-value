...
import type { KValueEntry, PostgreSQLOptions } from '../abstraction/adapter'
...
export class PostgreSQLAdapter extends Adapter {
  ...
  public async delete (key: string | string[]): Promise<void> {
    ...
    if (Array.isArray(key)) {
      ...
      await this.driver.knex.table(this.options.table).whereIn('key', key).delete()
      key.forEach((k) => { this.memcache.delete(k) })
    } else {
      // Delete Single ID and Purge Cache
      await this.driver.knex.table(this.options.table).where({ key: key }).delete()
  ...
  public async get (key: string | string[], options?: GetOptions & CacheOptions): Promise<unknown | Array<{ key: string; value: unknown; }>> {
    ...
    if (Array.isArray(key)) {
      ...
      const kValueTables = await this.driver.knex(this.options.table).select('*').whereIn('key', key) as KValueEntry[]
      ...
      const state = await this.driver.knex(this.options.table).select('*').where({ key: key }).first() as KValueEntry
      const context = super._import(state)
      // Default Invalid or Undefined Contexts
  ...
  public async has (key: string | string[]): Promise<boolean | Array<{ key: string; has: boolean; }>> {
    ...
    if (Array.isArray(key)) {
      ...
      const kValueEntries = await this.driver.knex(this.options.table).select('key').whereIn('key', key) as KValueEntry[]
      ...
      return await this.driver.knex(this.options.table).select('key').where({ key: key }).first() !== undefined
    }
  }
  ...
  public async keys (limits?: LimiterOptions): Promise<string[]> {
    let kValueTables = (await this.driver.knex(this.options.table).select('key') as KValueEntry[]).map((v) => v.key)
  ...
  public async set (key: string | string[], value: unknown, options?: SetOptions): Promise<void> {
    ...
    if (Array.isArray(key)) {
      ...
      for (const k of key) {
        ...
        }).into(this.options.table).onConflict('key').merge()
      }
    } else {
      // Set Single Key
      // Cache if Applicable
